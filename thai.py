...
def Print_Logo():
    ...
    Logo = '''
                                 '''+GREEN+'''['''+MAGENTA+'''*'''+GREEN+''']'''+YELLOW+'''เลือกหัวข้อ '''+WHITE+''':
              '''+GREEN+'''['''+MAGENTA+'''1'''+GREEN+''']'''+YELLOW+'''Get Information | '''+GREEN+'''['''+MAGENTA+'''2'''+GREEN+''']'''+YELLOW+'''Check Plugin | '''+GREEN+'''['''+MAGENTA+'''3'''+GREEN+''']'''+YELLOW+'''Brute Forcer
...
def Main():
    Clear()
    Print_Logo()
    op = input(GREEN+'กรุณาใส่ ใส่ตัวเลข  > '+YELLOW)
    if op != '':
        if int(op) <= 3:
            if int(op) == 1:
                Clear()
...
def BruteForcer():
    print('\n\n '+YELLOW+'['+GREEN+'!'+YELLOW+']'+BLUE+'Please Enter The Target ( ไม่ต้องใส่ https:// ) Url\n')
    Target = input(MAGENTA+'  > '+GREEN)
    Url = UrlTools(Target)
    print('\n')
